...
Scripts to verify are loaded on the page. Can be simple strings or detailed objects.
...
**How it works:**
- The tool searches page source code for these snippets
- Matches both external script `src` attributes and inline script content
- Reports which scripts are missing

#### `events` (array of objects or strings)

DataLayer events to track. Can be simple strings or detailed objects.
...
**How it works:**
- Tool monitors `window.dataLayer.push()` calls
- Tracks which events fire during page load
...
# Reports are generated as JSON files
# Use these in your pipeline
```

### Customizing Reports

Edit `lib/reporter.js` to:
...
1. Create new file in `lib/` (e.g., `lib/custom-checker.js`)
2. Import in `test-runner.js`
3. Call your checker function
4. Generate reports with Reporter
...
3. Check generated JSON reports for detailed data
